...
- **No NAT Gateway.** The backend EC2 instance sits in a *public* subnet with a direct route to the Internet Gateway, not a private subnet behind a NAT Gateway. The security group is the enforcement boundary instead (ALB-only inbound, 443/DNS-only outbound) — this was a deliberate cost/complexity trade-off for a dev/demo-stage project (a NAT Gateway alone runs ~$40+/month).
...
- **No NAT Gateway; EC2 in a public subnet.** The security group is the sole network boundary: inbound allowed only from the ALB's security group (never a CIDR, never SSH), outbound restricted to 443/tcp plus DNS (53/tcp+udp) scoped to the VPC CIDR only. This trades one layer of defense-in-depth (no network path to the instance at all) for real monthly savings, on the premise that the security group is airtight. Revisit for a environment with a lower risk tolerance for that trade-off.
